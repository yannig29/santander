...
{
  Data[which(is.na(Data[, columns[i]])), columns[i] ]<- mean(Data[, columns[i]]%>%unlist%>%as.numeric, na.rm=T)
}
...
glm0 <- glmnet(x=dataxgb0, y=log(Data[s,]$target), family=c("gaussian"), offset=NULL, alpha = 1, nlambda = 100,
       standardize = TRUE, intercept=T)

plot(glm0, label=T, xvar='lambda')

glm0$beta

glm0cv <- cv.glmnet(x=dataxgb0, y=log(Data[s,]$target), family=c("gaussian"), offset=NULL, alpha = 1, nlambda = 100,
...
reg0.forecast <- exp(reg0$coefficients[1]+dataxgb1[,which(glm0$beta[,which.min(l)]!=0)]%*%reg0$coefficients[-1])
...
xgb0 <- xgboost(params=list(subsample=0.5,eta = 0.05,max.depth = 6, colsample_bytree=1),
                data =dataxgb0_1 , label=log(Data[s,]$target),
                , nthread = 4, objective = "reg:linear", nround = 1000,
...
xgb0 <- xgboost(params=list(subsample=0.5,eta = 0.05,max.depth = 6, colsample_bytree=1),
                data =dataxgb0 , label=log(Data$target),
                , nthread = 4, objective = "reg:linear", nround = 1000,
                booster = "gbtree", verbose = 0)
